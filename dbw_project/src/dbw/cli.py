...
from .bakefile import BakeFileGenerator, setup_buildx_builder
from .compose import create_base_dockerfile, generate_compose_file
...
def launch(
    ...
    no_gpu: bool = typer.Option(False, "--no-gpu", help="Disable GPU support"),
) -> None:
    ...
    try:
        ...
        dockerfile_path = create_base_dockerfile(worktree_path, base_image)
...
def list() -> None:
    ...
    try:
        ...
        result = subprocess.run(
            ["docker", "ps", "--filter", "name=dbw_", "--format", "table {{.Names}}\t{{.Status}}\t{{.Ports}}"],
            capture_output=True,
            text=True,
            check=False,
        )
...
def prune(
    ...
    try:
        ...
        subprocess.run([
            ...
            "--filter", f"until={days * 24}h",
            "--filter", "label=dbw=true",
        ], check=False)
