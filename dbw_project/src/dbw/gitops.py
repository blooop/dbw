...
def clone_repo(owner: str, repo: str, bare_path: Path) -> git.Repo:
    ...
    try:
        ...
        if "Repository not found" in str(e) or "could not read" in str(e):
            raise NetworkError(f"Repository {owner}/{repo} not found or access denied")
        raise NetworkError(f"Failed to clone {owner}/{repo}: {e}")
    except OSError as e:
        if e.errno == 28:  # ENOSPC
            raise GitError(f"Not enough disk space to clone {owner}/{repo}")
        raise GitError(f"Failed to clone {owner}/{repo}: {e}")
...
def ensure_worktree(git_repo: git.Repo, repo: str, branch: str) -> Path:
    ...
    try:
        ...
        try:
            ...
            raise BranchMissing(f"Branch '{branch}' not found in remote repository")
        ...
        if "already exists" in str(e):
            ...
            return worktree_path
        raise GitError(f"Failed to create worktree for {branch}: {e}")
