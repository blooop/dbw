...
from dbw.config import ensure_dirs
...
@pytest.fixture
def mock_workspaces_dir(temp_dir: Path) -> Generator[Path, None, None]:
    """Mock the workspaces directory."""
    workspaces_dir = temp_dir / "workspaces"
    workspaces_dir.mkdir(parents=True, exist_ok=True)
    
    with patch("dbw.config.get_workspaces_dir", return_value=workspaces_dir):
        yield workspaces_dir


@pytest.fixture
def mock_extensions_dir(temp_dir: Path) -> Generator[Path, None, None]:
    """Mock the extensions directory."""
    extensions_dir = temp_dir / "extensions"
    extensions_dir.mkdir(parents=True, exist_ok=True)
    
    with patch("dbw.config.get_extensions_dir", return_value=extensions_dir):
        yield extensions_dir


@pytest.fixture
def mock_cache_dir(temp_dir: Path) -> Generator[Path, None, None]:
    """Mock the buildx cache directory."""
    cache_dir = temp_dir / "cache"
    cache_dir.mkdir(parents=True, exist_ok=True)
    
    with patch("dbw.config.get_buildx_cache_dir", return_value=cache_dir):
        yield cache_dir


@pytest.fixture
def mock_bare_repo(temp_dir: Path) -> Generator[git.Repo, None, None]:
...
@pytest.fixture
def mock_worktree(temp_dir: Path, mock_bare_repo: git.Repo) -> Generator[Path, None, None]:
...
@pytest.fixture
def extension_cache(mock_extensions_dir: Path) -> ExtensionCache:
    """Create an ExtensionCache instance with mocked directory."""
    return ExtensionCache()


@pytest.fixture
def sample_extension(mock_extensions_dir: Path) -> Path:
